package com.bptn.course._19_junit._coding_44;
...
public class BinarySearch {
  ...
  public static int binarySearch(ArrayList<Integer> list, int key) {
    ...
    while (low <= high) {
      ...
      // If the element at the middle index is less than the key, move the low pointer to mid + 1
      if (list.get(mid) < key) {
        low = mid + 1;
      }
      
      // If the element at the middle index is greater than the key, move the high pointer to mid - 1
      else {
        high = mid - 1;
      }
    }
